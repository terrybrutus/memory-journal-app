...
import { useUpdateMemory, useAllTags, useAllCategories, useAddTag, useAddCategory } from '@/hooks/useQueries';
import { useFileUpload } from '@/blob-storage/FileStorage';
import { ContentType, FileType, type MediaFile, type MemoryEntry } from '@/backend';
...
export default function EditMemoryDialog({ memory, open, onOpenChange }: EditMemoryDialogProps) {
    ...
    const [contentType, setContentType] = useState<ContentType>(memory.contentType);
    const [existingFiles, setExistingFiles] = useState<MediaFile[]>(memory.mediaFiles);
    ...
    useEffect(() => {
        if (open) {
            ...
            setContentType(memory.contentType);
            setExistingFiles(memory.mediaFiles);
            setNewFiles([]);
        }
    }, [open, memory]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const allMediaFiles = [...existingFiles, ...uploadedFiles];
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto vhs-dialog">
                <DialogHeader>
                    <DialogTitle className="vhs-text">Edit Memory</DialogTitle>
                </DialogHeader>
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        <Label className="vhs-text-small">Existing Media</Label>
                ...
                <DialogFooter>
                    ...
                    <Button
                        ...
                        className="vhs-button"
                    >
                        {isUpdating || isUploading ? 'Updating...' : 'Update Memory'}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
